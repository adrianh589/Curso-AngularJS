...
if (empty($cliente['nombre']) || empty($cliente['correo'])) {
	echo json_encode(array('err' => true, 'Mensaje' => 'Los campos nombre y correo son obligatorios.'));
	exit;
}
...
if ($esActualizacion) {
	...
	if ($stmt === false) {
		$respuesta = array('err' => true, 'Mensaje' => $conexion->error);
	} else {
		$id = (int) $request['id'];
		$stmt->bind_param(
			"sssssssi",
		...
		if ($stmt->execute()) {
			...
			$respuesta = array('err' => true, 'Mensaje' => $stmt->error);
		}
